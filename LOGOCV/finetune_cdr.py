### LOOCV split - Binary Classification 
...
ds_path=f'/data/finetuning/01-BinaryClassification/fullseq/loocv/tokenized_datasets/{ds_name}_{tokenizer_name}'

print(ds_path)
mhcs=os.listdir(ds_path)
print(mhcs)
loocv_binary_results=[]
for mhc in tqdm(mhcs):
    
    # Initialize aim_callback
    aim_callback = AimCallback(experiment=f'{model_name}_{ds_name}_{mhc}', repo='/data/aim')
    ...
    tokenized_datasets = load_from_disk(f'{ds_path}/{mhc}')
    tokenized_datasets.set_format('torch')

    training_args = TrainingArguments(
        output_dir=f"models/{ds_name}/{model_name}/{mhc}",
        logging_dir=f"logs/{ds_name}/{model_name}/{mhc}",
        ...
        eval_steps=100,
        logging_steps=100,
        num_train_epochs=4, # increased from 4
    ...
    print(f"Training a {model_name} model on {mhc}")
    trainer.train()
    predictions = trainer.predict(tokenized_datasets['test'])
    ####
    result_metric=predictions.metrics
    result_metric['mhc']=mhc
    loocv_binary_results.append(result_metric)
    print(result_metric)
    ####
    ...
    pred_df.to_csv(f'{outpath}/{mhc}.csv', index=False)
    
final_binary_results=pd.DataFrame(loocv_binary_results)
print(final_binary_results)
pred_path=f"/data/finetuning/01-BinaryClassification/fullseq/loocv/predictions"
